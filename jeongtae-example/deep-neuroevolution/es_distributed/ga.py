...
def run_master(master_redis_cfg, log_dir, exp):
    ...
    while True:
        ...
        # 위의 EpLenMean과 동일한 에러 발생으로 인한 임시적인 주석처리.
...
def run_worker(master_redis_cfg, relay_redis_cfg, noise, *, min_task_runtime=.2):
    ...
    while True:
        ...
        if rs.rand() < config.eval_prob:
            ...
            # ValueError: too many values to unpack 떠서 temp_array라는 빈 array 추가.
            eval_rews, eval_length, temp_array = policy.rollout(env)  # eval rollouts don't obey task_data.timestep_limit
