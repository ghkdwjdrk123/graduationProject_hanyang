...
class Policy:
    ...
    # eval()은 안에 들어가는 표현식을 그대로 실행하는 함수.
    # 이게 아마 초기상태로 돌아가는 함수이지 않을까?
...
class GAPolicy_Pen(Policy):
    ...
    # 분명 이 DNN을 통과하면 여러 결과값이 나올 것이다. 
    # (구체적으로는 num_actions와 동일한 갯수의 결과값.)
    # tf.argmax는?
    def _make_net(self, o):
        x = o

        """
        x = layers.fully_connected(x, num_outputs=32, activation_fn=None, scope='fc1')
        x = layers.fully_connected(x, num_outputs=32, activation_fn=None, scope='fc2')
        a = layers.fully_connected(x, num_outputs=self.num_actions, activation_fn=None, scope='out')
        """
        ...
        return tf.argmax(a,1)
        print("makenet", a.get_shape(), tf.argmax(a, 1).get_shape())
    ...
    # Dont add random noise since action space is discrete
    def act(self, train_vars, random_stream=None):
        return np.clip(self._act(train_vars), -2, 2)
