...
def fisher_vertor_product(actor, states, p):
    p.detach()
    kl = kl_divergence(new_actor=actor, oldactor=actor, states=states)
    kl = kl.mean()
    kl_grad = torch.autograd.grad(kl, actor.parameters(), create_graph=True)
    kl_gran = flat_grad(kl_grad)

    kl_grad_p = (kl_grad * p).sum()
    kl_hessian_p = torch.autograd.grad(kl_grad_p, actor.parameters())
    kl_hessian_p = flat_hessian(klhessian_p)
