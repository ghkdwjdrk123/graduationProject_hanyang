...
# main.py의 master()에서 오게 됨.
# exp는 .json파일에 적혀있는 내용들.
...
def run_master(master_redis_cfg, log_dir, exp):
    ...
    tlogger.start(log_dir)          # 아마 undo를 위한 log로 추정.
    config, env, sess, policy = setup(exp, single_threaded=False)
    master = MasterClient(master_redis_cfg) # redis를 위한 코드. redis는 병렬처리를 위해 사용했을 것으로 추측.
    ...
    population = []
    population_size = exp['population_size']    # 기본은 10
    num_elites = exp['num_elites']              # 기본은 1
    population_score = np.array([])             # 아마 각 reward를 저장하는 array?

    while True:
        step_tstart = time.time()
        theta = policy.get_trainable_flat()     # 현재 policy는 GAAtariPolicy
        ...
        while num_episodes_popped < config.episodes_per_batch or num_timesteps_popped < config.timesteps_per_batch:     # 뽑힌 episodes나 timesteps가 timesteps_per_batch보다 많을때 반복?
            # Wait for a result
...
def run_worker(master_redis_cfg, relay_redis_cfg, noise, *, min_task_runtime=.2):
    ...
    while True:
        ...
        if rs.rand() < config.eval_prob:        # 얘는 rand()의 결과가 config.eval_prob보다 작아서 noise가 없고
        ...
        else:                                   # 얘는 rand()의 결과가 config.eval_prob보다 커서 noise가 있고?
            # Rollouts with noise
